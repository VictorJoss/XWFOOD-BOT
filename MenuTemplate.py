...
class MenuTemplate:
     ...
     @staticmethod
     def get_products_vew_from_table_values(table: List[List[str]]):
          table.pop(0)
          keyboard = [[MenuTemplate.BACK_BUTTON]]
          products_str = "*📋 Menú de hoy*"
          products_str += "\n" + (len(products_str) * "-") + "\n"
          for row in table:
               row_str = " | ".join(row)
     ...
     @staticmethod
     def get_my_orders_view(table: List[List[str]], username: str) -> Tuple[str, InlineKeyboardMarkup]:
